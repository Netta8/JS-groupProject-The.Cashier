...
function createCashCounter() {
  ...
  return function cashCounter(price, costumerPaid) {
    //anonymous function. By returning the anonymous function, a closure created, that retains the reference to the cashBox array and its state, allowing the returned function to access and modify it even after createCashCounter has completed execution.
    ...
    for (let i = 0; i < cashBox.length; i++) {
      let cashInCashier = Object.keys(cashBox[i])[0]; // We get the current note from the array of notes
      ...
      if (count > 0 && cashBox[i][cashInCashier] >= count) {
        ...
        result.push({ [cashInCashier + " Euro"]: count }); // We create the object with the current note as the key and the count as a value
        changeForTheCostumer -= count * cashInCashier;
        cashBox[i][cashInCashier] -= count; //we subtract the number of notes we used for the original array
